...
def BFI_solver(Separation_Method,k=.5,C=0,gamma=0):
    k = int(k*1000000)
    #k = 500
    recharge = 1
    for i in range(k,661300):
        if recharge != 3438000.0:
            k = (i + 1)/1000000
...
#            del discharge_df['Date']
            ...
            recharge = round((recharge / 100),0)
            recharge = recharge * 100
            discharge_df.to_csv('turning_pints.csv')
